...
def ALTM():
    print('Do you want to claim the account?')
    s = input('> ')
    if s == 'y' or s == 'yes':
        #os.system('python3 alt_maker.py claim-account')
        os.system('py -3 start_the_alt_maker.py' if os.name == "nt" else 'python3 start_the_alt_maker.py')
    elif s == 'num':
        print('How many alts do you want to make?')
        num = input('> ')
        int_num = int(num)
        total_num = 0
        while total_num <= int_num:
            os.system('py -3 start_the_alt_maker.py' if os.name == "nt" else 'python3 start_the_alt_maker.py')
            total_num += 1
            print('Waiting so that you dont hit the rate limit')
            time.sleep(60)
    elif s == 'n' or s == 'no':
